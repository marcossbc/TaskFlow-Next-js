...
export default function Sidebar() {
  ...
  const [isOpen, setIsOpen] = useState(false); // Xaaladda moobaylka
  ...
  return (
    <>
      {/* BADANKA HAMBURGER-KA: Wuxuu u muuqanayaa oo kaliya Moobaylka (md:hidden) */}
      <button
        onClick={toggleSidebar}
        type="button"
        className="fixed top-4 left-4 z-50 p-2.5 rounded-xl bg-[#0D1220] border border-gray-800 text-gray-200 md:hidden hover:bg-gray-900 active:scale-95 transition-all shadow-xl"
      >
        {isOpen ? <X size={20} /> : <Menu size={20} />}
      </button>

      {/* BACKDROP SHADOW: Marka moobaylka uu u furanyahay sidebar-ka, gadaasheyda madoobaataa */}
      {isOpen && (
        <div
          onClick={toggleSidebar}
      ...
      {/* SIDEBAR CONTAINER: Desktop-ka waa caadi, Moobaylkana waa la simayaa (Slide-in) */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 h-screen flex flex-col bg-[#070A13] border-r border-indigo-500/10 text-gray-200 font-sans select-none overflow-hidden transition-transform duration-300 ease-in-out md:sticky md:translate-x-0 ${
      ...
      >
        ...
        {/* 1. BRAND HEADER */}
        <div className="px-6 h-24 flex items-center justify-between relative z-10 pt-4 md:pt-0">
          ...
          {navItems.map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                onClick={() => setIsOpen(false)} // Marka la gujiyo link moobaylka, sidebar-ku ha xirmo
